...
############################# Plotting ###############################
######################################################################
def plot_results(h_pred, h_DataSet, x, t, x0, tb, lb, ub, x_f, t_f):
    plt.ioff()
    ...
    H_true = griddata(X_star, h_DataSet[:, 0], (X, T), method='cubic')
    ...
    ax0.set_zlabel('|h(x,t)|', fontdict={'fontsize': 12})
    ax0.view_init(30, 45)
    ...
    for i, slice_idx in enumerate(slices):
        ...
        ax.plot(x, H_pred[slice_idx, :], 'r--', linewidth=2, label='Prediction')
        ax.set_xlabel('$x$')
        ax.set_ylabel('$|h(x,t)|$')
...
def plot_results0(h_pred, h_DataSet, x,t,x0,tb,lb,ub, x_f,t_f):
...
#        ax1.plot(X_f[:,1], X_f[:,0], 'rx', label = 'Data (initerior) (%d points)' % (X_f.shape[0]), markersize = 1, alpha=.5, clip_on = False)    
    ...
    ax1.set_xlabel('$t$')
    ax1.set_ylabel('$x$')
    ax1.set_title('$|h(x,t)|$', fontsize = 10)
...
def plot_results1(h_pred, h_DataSet, x, t, x0, tb, lb, ub, x_f, t_f):
    plt.ioff()
    ...
    ax1.plot(x, H_pred[75, :], 'r-.', linewidth=2, markersize=4, label='Prediction')
    ax1.set_xlabel('$x$')
    ...
    ax2.plot(x, H_pred[100, :], 'r:', linewidth=2, markersize=4, label='Prediction')
    ...
    ax3.plot(x, H_pred[125, :], 'r--', linewidth=2, markersize=4, label='Prediction')
    ax3.set_xlabel('$x$')
...
def plot_error(h_pred, h_DataSet, x, t, x0, tb, lb, ub):
    plt.ioff()
    ...
    ax1.plot_surface(X, T, H_pred, facecolors=plt.cm.viridis(H_pred), cmap=cmap_result, antialiased=True)
    ax1.set_title('Prediction', fontsize=12, fontweight='bold')
    ax1.set_xlabel('t', fontsize=10)
    ax1.set_ylabel('x', fontsize=10)
    ax1.set_zlabel('|h(x,t)|', fontsize=10)
    ax1.view_init(30, 45)
...
def plot_error0(h_pred, h_DataSet, x, t, x0, tb, lb, ub):
    plt.ioff()
...
def plot_results_3d(h_pred, h_DataSet, x, t):
    X, T = np.meshgrid(x, t)
    H_pred = griddata(np.hstack((X.flatten()[:, None], T.flatten()[:, None])), h_pred[:, 0], (X, T), method='cubic')
    H_true = griddata(np.hstack((X.flatten()[:, None], T.flatten()[:, None])), h_DataSet[:, 0], (X, T), method='cubic')

    fig = plt.figure(figsize=(14, 7))
    ax1 = fig.add_subplot(121, projection='3d')
    ax1.plot_surface(X, T, H_pred, cmap='viridis')
    ax1.set_title('Predicted - |h(x,t)|', fontsize=16)
    ...
    ax2.set_title('Dataset - |h(x,t)|', fontsize=16)
...
def plot_error_heatmap(h_pred, h_DataSet, x, t):
    X, T = np.meshgrid(x, t)
    H_pred = griddata(np.hstack((X.flatten()[:, None], T.flatten()[:, None])), h_pred[:, 0], (X, T), method='cubic')
    H_true = griddata(np.hstack((X.flatten()[:, None], T.flatten()[:, None])), h_DataSet[:, 0], (X, T), method='cubic')
    H_error = np.abs(H_pred - H_true)
...
def plot_results_3d_heat(u_pred, u_true, X, T):
    # Flatten the u_pred and u_true arrays
    u_pred_flattened = u_pred.flatten()
    u_true_flattened = u_true.flatten()

    # Print shapes for debugging
    print("Shape of X.flatten():", X.flatten().shape)
    print("Shape of u_pred_flattened:", u_pred_flattened.shape)

    # Ensure that the number of grid points matches the number of values in u_pred
    assert X.flatten().shape[0] == u_pred_flattened.shape[0], "Mismatch between number of grid points and u_pred values."

    U_pred = griddata(np.hstack((X.flatten()[:, None], T.flatten()[:, None])), u_pred_flattened, (X, T), method='cubic')
    U_true = griddata(np.hstack((X.flatten()[:, None], T.flatten()[:, None])), u_true_flattened, (X, T), method='cubic')
    ...
    ax1.set_xlabel('x', fontsize=14)
    ax1.set_ylabel('t', fontsize=14)
    ax1.set_zlabel('u', fontsize=14)
    ax1.view_init(30, 45)  # Adjust view angle for better visualization

    ax2 = fig.add_subplot(122, projection='3d')
    ax2.plot_surface(X, T, U_true, cmap='viridis')
    ax2.set_title('U simulate', fontsize=16)
    ax2.set_xlabel('x', fontsize=14)
    ax2.set_ylabel('t', fontsize=14)
    ax2.set_zlabel('u', fontsize=14)
    ax2.view_init(30, 45)  # Adjust view angle for better visualization

    return fig

def plot_error_heatmap_heat(u_pred, u_true, X, T):
    # Flatten the u_pred and u_true arrays
    u_pred_flattened = u_pred.flatten()
    u_true_flattened = u_true.flatten()
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    sns.heatmap(U_error, ax=ax, cmap='inferno', cbar_kws={'label': 'Error Magnitude'})
    ax.set_title('Absolute Error Heatmap')
    ax.set_xlabel('x')
    ax.set_ylabel('t')
...
def plot_initial_condition(x, u0):
    fig = plt.figure(figsize=(10, 7))
    ax = fig.add_subplot(111, projection='3d')
    ax.plot(x, [0]*len(x), u0, 'r-', linewidth=2, label='Initial Condition')
    ax.set_xlabel('x')
    ax.set_ylabel('t')
    ax.set_zlabel('u')
    ax.set_title('Initial Condition u(x,0)')
    ax.legend()
    ax.view_init(30, 45)  # Adjust view angle for better visualization
    return fig

def plot_coefficients_distribution(coeffs):
    fig = plt.figure(figsize=(10, 7))
    ax = fig.add_subplot(111, projection='3d')
    x = range(len(coeffs))
    y = [0]*len(coeffs)
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Value')
    ax.set_title('Coefficients of the Polynomial')
    ax.view_init(30, 45)  # Adjust view angle for better visualization
    return fig

def plot_coefficients_histogram(coeffs):
    fig = plt.figure(figsize=(10, 7))
    ax = fig.add_subplot(111, projection='3d')
    hist, bins = np.histogram(coeffs, bins=20)
    x = [0.5*(x_+x_next) for x_, x_next in zip(bins[:-1], bins[1:])]
    y = [0]*len(x)
    z = [0]*len(x)
    dx = [0.5]*len(x)
    dy = [0.5]*len(x)
    dz = hist
    ax.bar3d(x, y, z, dx, dy, dz, shade=True, color='green')
